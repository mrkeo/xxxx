...
const GeneratePdf = ({ projects }: ProjectsProps) => {
  const createDocument = () => {
    if (typeof window !== "undefined") {
      ...
      doc.setFontSize(8);
      doc.text(
        `document generated on ${moment().format("DD.MM.YYYY")} with jsPDF`,
        55.5,
        (y += 4)
      ); // 33.8
      doc.text("source: ", 55.5, (y += 3.7)); // 37.5
      doc.textWithLink(`${window.location.href}`, 65.2, y, {
      ...
      for (let i = 0; i < projects.length; i++) {
        ...
        doc.text(
          `{ ${i + 1} - ${moment(projects[i].date).format("DD.MM.YYYY")} }`,
          leftMargin,
          y
        );
        doc.setTextColor(0);
        ...
        doc.setFontSize(12);
        doc.text(
          `${projects[i].title} (${projects[i].service})`,
          leftMargin,
          y
        );
        y += 5;
        ...
        let text =
          projects[i].desc.length > 250
            ? projects[i].desc.substr(0, 250)
            : projects[i].desc;
        ...
        doc.textWithLink(
          `${window.location.href.slice(0, -1)}${projects[i].slug}`,
          24.5,
          y,
          { url: `${window.location.href.slice(0, -1)}${projects[i].slug}` }
        );
        y += 10;
        doc.setFontSize(12);
        if (i === 4 || i === 10 || i === 16) {
      ...
      doc.save(`${title.replace(/\s/g, "-")}-projects.pdf`);
    }
  };
      ...
      <Tooltip
        title={
          <div className="pdf-link-tooltip">
            ...
            <span
              className="iconify"
              data-icon="grommet-icons:document-pdf"
        ...
        arrow
      >
        <a className="download-pdf-anchor" onClick={createDocument.bind(this)}>
          <span
            className="iconify"
            style={{ width: "6rem", height: "6rem", cursor: "pointer" }}
            data-icon="bi:download"
            data-inline="false"
          />
        </a>
      </Tooltip>
